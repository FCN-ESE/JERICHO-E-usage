# -*- coding: utf-8 -*-
"""* * @Author: Jan Priesmann, Lars Nolting, Christina Kockel, and Aaron Praktiknjo  * @Date: 2020-10-19  *"""
...
def calculate_ts():
    ...
    commerce_subsectors = ['agriculture','trade','finance','public']
    ...
    sectoral_regional_factors = pd.read_excel(dirname + '/Input Data/Commerce/sectoral_regional_factors_NUTS2.xlsx', engine="openpyxl", sheet_name='sectoral_regional_factors_NUTS2', index_col=[0])

    commerce_standard_load_profiles = pd.read_excel(dirname + '/Input Data/Commerce/commerce_standard_load_profiles.xlsx', engine="openpyxl", sheet_name='commerce_standard_load_profiles', index_col=[0]).reset_index(drop=True)
    ...
    ageb_end_use = pd.read_excel(dirname + '/Input Data/General/ageb_end_use_2019.xlsx', engine="openpyxl", sheet_name='com', skiprows=[0,1,2], index_col=[0])
    ...
    end_use_con_eff = end_use_con_eff.loc[end_use_con_eff.Sector == 'Services']
    ...
    annual_FE_per_end_use_and_sector_NUTS2 = {final_energy: {sector: pd.DataFrame(0.0, index=NUTS2_regions, columns=end_use_translation) for sector in commerce_subsectors} for final_energy in final_energy_translation}
    ...
    for final_energy in final_energy_translation:
        ...
        for end_use in end_use_translation:
            ...
            for sector in commerce_subsectors:
        ...
        for end_use in end_use_translation:        
            ...
            for sector in commerce_subsectors:
                ...
                for NUTS2 in NUTS2_regions:
                    ...
                    hourly_FE_per_end_use_kw = commerce_standard_load_profiles['all_commerce'].multiply(total_FE_per_end_use_kwh)
    ...
    total_UE_for_space_heat_before_kwh = hourly_UE_NUTS2_kw['Space Heat'].sum().sum()
    ...
    scaling_factor = total_UE_for_space_heat_before_kwh / total_UE_for_space_heat_after_kwh
    ...
    for end_use in end_use_translation:      
        ...
        hourly_UE_NUTS2_kw[end_use].to_csv(dirname + '/../Final Data/Commerce/nuts2_hourly_com_' + end_use + '_kw.csv', sep=';', encoding="ISO-8859-1", decimal=',')
